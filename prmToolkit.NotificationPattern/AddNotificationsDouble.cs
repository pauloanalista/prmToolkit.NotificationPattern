...
namespace prmToolkit.NotificationPattern
{
    public partial class AddNotifications<T> where T : Notifiable
    {
        ...
        /// <summary>
        /// Dada um double, adicione uma notificação se for igual a zero
        /// </summary>
        ...
        public AddNotifications<T> IfEqualsZero(Expression<Func<T, double>> selector, string message = "")
        {
        ...
        public AddNotifications<T> IfEqualsZero(double val, string objectName, string message = "")
        {
            if (val == 0)
